...
class DokumenKaryawanExport implements FromView, WithTitle, WithStyles, ShouldAutoSize, WithEvents
{
    protected $dokumenKaryawan;
    protected $filter;

    public function __construct($dokumenKaryawan, $filter = null)
    {
        $this->dokumenKaryawan = $dokumenKaryawan;
        $this->filter = $filter;
    }

    public function view(): View
    {
        return view('dokumen-karyawan.export-excel', [
            'dokumenKaryawan' => $this->dokumenKaryawan,
            'filter' => $this->filter
        ]);
    }
    ...
    public function styles(Worksheet $sheet)
    {
        return [
            ...
            'A1:L' . (count($this->dokumenKaryawan) + 1) => [
                'borders' => [
                    'allBorders' => [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                foreach ($this->dokumenKaryawan as $dokumen) {
                ...
                if ($this->filter) {
                    $filterRowStart = count($this->dokumenKaryawan) + 3;
